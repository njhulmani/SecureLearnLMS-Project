...
function StudentDashboard() {
  ...
  const fetchContinueWatching =
    useCallback(async () => {
      ...
      try {
        const response = await api.get('/api/continue-watching/');
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100">
        ...
        <div className="relative">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-400 to-blue-600 text-sm font-bold text-white"> S </div>
                  <div>
                    <h1 className="text-sm font-semibold text-white"> Student Dashboard </h1>
                    <p className="text-xs text-cyan-300/80 uppercase tracking-wider"> Student </p>
                  </div>
                </div>
          ...
          <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {/* PAGE HEADER */}
            <div className="mb-8">
              <h2 className="text-4xl sm:text-5xl font-bold text-white mb-2"> My Courses </h2>
              <p className="text-slate-400 text-lg"> Access your enrolled courses and track your learning progress </p>
            </div>
            ...
            <div>
              {loading ? (
                ...
                <div className="rounded-2xl border-2 border-dashed border-white/15 bg-slate-950/40 p-12 text-center">
                  ...
                  <p className="text-slate-400 mb-6"> Your admin will assign courses to you when they're available. </p>
                  <button
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/75 px-4 backdrop-blur-sm">
            <div className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-2xl shadow-cyan-950/40">
              ...
              <div className="flex items-start justify-between gap-4 border-b border-white/10 px-6 py-5">
                <div>
                  <p className="text-sm uppercase tracking-[0.3em] text-cyan-300/80">Student Profile</p>
...
function CourseCard({course, continueData, onViewCourse}) {

  const courseId = course.id || course.course_id;
  const courseTitle = course.course || course.title || 'Untitled Course';
  ...
  return (
    ...
    <div className="group rounded-xl border border-white/10 bg-slate-900/50 overflow-hidden shadow-lg hover:shadow-2xl hover:border-cyan-300/20 transition-all duration-300 hover:-translate-y-1 flex flex-col">
      ...
      <div className="p-6 flex flex-col flex-1">
        {/* TITLE */}
        <h3 className="text-xl font-bold text-white mb-2 line-clamp-2 group-hover:text-cyan-300 transition"> {courseTitle} </h3>
        ...
        {continueData ? (
          <div className="mt-auto">
            <div className="mb-3 rounded-lg border border-cyan-400/20 bg-cyan-500/10 p-3">
              <p className="text-xs uppercase tracking-wider text-cyan-300 mb-1">Continue Learning</p>
              <p className="text-sm font-medium text-white line-clamp-1">{continueData.video_title}</p>
            </div>

            <button
              onClick={() =>
                window.location.href = `/watch-course/${courseId}/${continueData.video_id}`
              }
